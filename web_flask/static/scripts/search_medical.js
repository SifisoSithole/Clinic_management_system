...
function openRecords(){
    ...
    table_rows.forEach((table_row) => {
        table_row.addEventListener('click', () => {
            $.ajax({
                url: 'https://healtheasehub.live/search/record/' + table_row.id,
                method: 'GET',
                success: (response) => {
                    $('.rows ul').remove();
                    ...
                    if (position !== 'Patient') {
                        document.getElementsByClassName('back-button')[1].addEventListener('click', () => {
                            $.ajax({
                                url: 'https://healtheasehub.live/accounts/remove/MedicalRecords/' + response.id,
                                method: 'delete',
                                success: (response) => {
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('medical').addEventListener('click', () => {
        ...
        let search;
        $.ajax({
            url: 'https://healtheasehub.live/search/MedicalRecords/' + string,
            method: 'GET',
            success: (response) => {
