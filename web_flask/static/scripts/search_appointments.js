    ...
    table_rows.forEach((table_row) => {
        table_row.addEventListener('click', () => {
            ...
            if (position === 'Receptionist' || position === 'Doctor') {
                url = 'http://localhost:5000/search/Appointments/' + table_row.id;
            } else {
                url = 'http://localhost:5000/search/MedicalRecords1/' + table_row.id;
            }
            $.ajax({
                url: url,
                method: 'GET',
                    ...
                    if (position === 'Receptionist' || position === 'Doctor') {
                        if (position === 'Receptionist') {
                            document.getElementsByClassName('back-button')[1].addEventListener('click', () => {
                                $.ajax({
                                    url: 'http://localhost:5000/accounts/remove/Appointments/' + response[0].id,
                                    method: 'delete',
                                    success: (response) => {
                            ...
                            document.getElementsByClassName('back-button')[2].addEventListener('click', () => {
                                $.ajax({
                                    url: 'http://localhost:5000/accounts/checkin/Appointments/' + response[0].id,
                                    method: 'post',
                                    success: (response) => {
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('medical').addEventListener('click', () => {
        ...
        $.ajax({
            url: 'http://localhost:5000/search/Appointments/' + string,
            method: 'GET',
            success: (response) => {
